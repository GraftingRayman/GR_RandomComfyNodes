"""
GRLoadAudioImageBatchWithDurationTest
----------------------------------
...
into Impact Pack's `ImpactQueueTrigger` node to auto-advance through a
whole folder and then stop cleanly:
  - remaining_count hits 0 on the last file and stays 0 forever after,
    so ImpactQueueTrigger's change-detection naturally stops firing.
  - incremental_audio mode does NOT wrap back to file 0 once exhausted
    (unlike a naive `counter % count`) - it clamps at the last file, so
    the one unavoidable extra trigger-tick just repeats the last file
    instead of restarting the whole batch.
  - batch_complete is True only on that extra tick (and beyond), so you
    can gate downstream save/output nodes off it to do zero real work
    on that tick.
...
try:
    ...
    _log = GRLogger("GRLoadAudioImageBatchWithDurationTest")
except Exception:
    class _FallbackLogger:
        def info(self, msg): print(f"[GRLoadAudioImageBatchWithDurationTest] {msg}")
        def warn(self, msg): print(f"[GRLoadAudioImageBatchWithDurationTest][WARN] {msg}")
        def error(self, msg): print(f"[GRLoadAudioImageBatchWithDurationTest][ERROR] {msg}")
    _log = _FallbackLogger()

# Where we persist incremental-mode counters between runs/restarts.
_STATE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".gr_audio_image_batch_duration_test_state")
...
def _list_audio_files(path, pattern):
    if not os.path.isdir(path):
        raise ValueError(f"GRLoadAudioImageBatchWithDurationTest: path does not exist or is not a directory: {path}")
    files = sorted(glob.glob(os.path.join(path, pattern)))
    if not files:
        raise ValueError(f"GRLoadAudioImageBatchWithDurationTest: no files matched pattern '{pattern}' in '{path}'")
    return files
...
class GRLoadAudioImageBatchWithDurationTest:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def load_batch(self, path, pattern, mode, index, seed, label,
        ...
        if mode == "single_audio":
            ...
            if counter >= count:
                # Already served every file - clamp on the last one instead of
                # wrapping back to file 0, and flag this as a no-op tick so
                # downstream nodes (and the queue trigger) can stand down.
                idx = count - 1
                batch_complete = True
                # counter intentionally left as-is (not incremented further)
            else:
                idx = counter
                _save_counter(path, pattern, label, counter + 1)

        if mode == "incremental_audio":
            remaining_count = 0 if batch_complete else max(0, count - (idx + 1))
        else:
            # single_audio / random: not a progressive batch, but still
            # reported for informational wiring if useful.
...
NODE_CLASS_MAPPINGS = {
    "GRLoadAudioImageBatchWithDurationTest": GRLoadAudioImageBatchWithDurationTest,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "GRLoadAudioImageBatchWithDurationTest": "GR Load Audio+Image Batch (Duration) [TEST]",
}
